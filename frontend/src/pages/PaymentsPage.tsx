...
import { nanoToDate, formatDate, formatCurrency } from '../utils/dates';
...
function getPaymentModeVariant(mode: string): 'default' | 'secondary' | 'outline' | 'destructive' {
...
export default function PaymentsPage() {
  ...
  const { staffRestricted, canBulkUpload } = useStaffRestrictions();
            ...
            <TableBody>
              {sorted.map((payment) => (
                <TableRow key={payment.id}>
                  <TableCell className="text-sm">{formatDate(nanoToDate(payment.date))}</TableCell>
                  <TableCell className="font-medium">{payment.client}</TableCell>
                  <TableCell>
                    <Badge variant={getPaymentModeVariant(payment.mode)} className="text-xs">
                      {payment.mode}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right font-semibold">{formatCurrency(payment.amount)}</TableCell>
                  <TableCell className="text-muted-foreground text-sm">{payment.referenceNumber || '—'}</TableCell>
                  <TableCell className="text-muted-foreground text-sm">{payment.site || '—'}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>

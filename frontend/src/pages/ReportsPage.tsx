...
import { useChallans, usePayments, usePettyCash, useClients } from '../hooks/useQueries';
...
function isInDateRange(nanos: bigint, range: { start: Date; end: Date } | null): boolean {
...
export default function ReportsPage() {
  ...
  const pettyCashRecords: PettyCash[] = useMemo(
    () => (pettyCashWithAttachments as PettyCashWithAttachments[]).map((p) => p.pettyCash),
  ...
  const siteNames: string[] = useMemo(
    () =>
      Array.from(
        new Set(
          [...challans.map((c) => c.site), ...payments.map((p) => p.site)].filter(
  ...
  const filteredChallans = useMemo(() => {
    return challans.filter((c) => {
      ...
      if (clientFilter !== ALL_CLIENTS && c.clientName !== clientFilter) return false;
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter((p) => {
      ...
      if (clientFilter !== ALL_CLIENTS && p.client !== clientFilter) return false;
  ...
  const handleDownloadAll = () => {
    exportReportToExcel({
      ...
      summary: {
        ...
        outstanding: totalRent - totalPayments > 0 ? totalRent - totalPayments : 0,
        advance: totalRent - totalPayments < 0 ? Math.abs(totalRent - totalPayments) : 0,
    ...
    <div className="container mx-auto px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="border rounded-lg p-4 bg-card">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">Total Rent</p>
        ...
        <div className="border rounded-lg p-4 bg-card">
          ...
          <p className="text-xl font-bold mt-1">{formatCurrency(cashReceived)}</p>
        </div>
        <div className="border rounded-lg p-4 bg-card">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">Online Received</p>
          <p className="text-xl font-bold mt-1">{formatCurrency(onlineReceived)}</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="challans">
        <TabsList>
          <TabsTrigger value="challans">Challans ({filteredChallans.length})</TabsTrigger>
                ...
                {filteredChallans.length === 0 ? (
                  ...
                  filteredChallans.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell>{formatCurrency(calculateChallanTotal(c))}</TableCell>
                      <TableCell>
                  ...
                  filteredPettyCash.map((r) => {
                    ...
                    return (
                      <TableRow key={r.date.toString()}>
                        ...
                        <TableCell>{formatCurrency(r.openingBalance)}</TableCell>
                        <TableCell>{formatCurrency(r.cashFromMd)}</TableCell>
                        <TableCell>{formatCurrency(r.transferFromCashEquivalents)}</TableCell>
                        <TableCell>{formatCurrency(r.cashReceivedAuto)}</TableCell>
            ...
            <div className="mt-4 p-4 border rounded-lg bg-muted/20 flex flex-wrap gap-6 text-sm">
              <div>
                ...
                <span className="font-semibold">
                  {formatCurrency(filteredPettyCash.reduce((s, r) => s + r.expenses, 0))}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">Total Cash from MD: </span>
                <span className="font-semibold">
                  {formatCurrency(filteredPettyCash.reduce((s, r) => s + r.cashFromMd, 0))}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">Total Cash Received: </span>
                <span className="font-semibold">
                  {formatCurrency(filteredPettyCash.reduce((s, r) => s + r.cashReceivedAuto, 0))}
                </span>
              </div>
            </div>
          )}

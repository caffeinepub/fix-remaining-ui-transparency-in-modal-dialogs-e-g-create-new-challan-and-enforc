...
function calcChallanTotal(challan: Challan): number {
  return challan.items.reduce((s, i) => s + i.quantity * i.rate * i.rentalDays, 0) + challan.freight;
}

function filterByPeriod(challans: Challan[], payments: Payment[], start: Date | null, end: Date | null, future: boolean): { challans: Challan[]; payments: Payment[] } {
  const now = new Date();
...
function calcMetrics(challans: Challan[], payments: Payment[]): PeriodMetrics {
  ...
  const cashReceived = payments.filter(p => p.mode.toLowerCase() === 'cash').reduce((s, p) => s + p.amount, 0);
  const onlineReceived = payments.filter(p => p.mode.toLowerCase() !== 'cash').reduce((s, p) => s + p.amount, 0);
...
export function calculateDashboardMetrics(
  ...
  const daily = calcMetrics(
    ...Object.values(filterByPeriod(challans, payments, todayStart, todayEnd, false)) as [Challan[], Payment[]]
  );
  const monthly = calcMetrics(
    ...Object.values(filterByPeriod(challans, payments, monthStart, null, false)) as [Challan[], Payment[]]
  );
  const allTime = calcMetrics(
    ...Object.values(filterByPeriod(challans, payments, null, null, false)) as [Challan[], Payment[]]
  );

  const futureChallans = challans.filter(c => nanoToDate(c.rentDate) > todayStart);
  const futureRevenue = futureChallans.reduce((s, c) => s + calcChallanTotal(c), 0);

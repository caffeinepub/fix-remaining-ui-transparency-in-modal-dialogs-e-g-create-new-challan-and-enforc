...
export async function generateAndPrintPettyCashPdf(
  ...
  const rows: [string, number, boolean][] = [
    ...
    ['Transfer from Cash Equivalents', record.transferFromCashEquivalents, false],
  ...
  for (const attachment of attachments) {
    try {
      ...
      try {
        ...
        const scale = Math.min((pw - 100) / embeddedImage.width, (ph - 100) / embeddedImage.height);
        ...
        try {
          ...
          const copiedPages = await pdfDoc.copyPages(attachPdf, attachPdf.getPageIndices());

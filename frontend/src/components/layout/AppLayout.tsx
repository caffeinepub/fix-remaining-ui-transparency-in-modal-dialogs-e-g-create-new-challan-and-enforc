    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-5 border-b" style={{ borderColor: 'var(--sidebar-border)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-sm leading-tight" style={{ color: 'var(--sidebar-fg)' }}>RentIQ</div>
            <div className="text-xs" style={{ color: 'var(--sidebar-muted)' }}>Udaipur</div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        ...
        {isAdmin && (
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors"
            style={{
              background: currentPage === 'access-management' ? 'var(--sidebar-active)' : 'transparent',
              color: currentPage === 'access-management' ? 'var(--sidebar-active-fg)' : 'var(--sidebar-fg)',
            }}
            onMouseEnter={e => {
              if (currentPage !== 'access-management') (e.currentTarget as HTMLElement).style.background = 'var(--sidebar-accent)';
            }}
            onMouseLeave={e => {
              if (currentPage !== 'access-management') (e.currentTarget as HTMLElement).style.background = 'transparent';
            }}
          >
            <ShieldCheck className="w-4 h-4 shrink-0" />
            <span>Access Management</span>
            {currentPage === 'access-management' && <ChevronRight className="w-3 h-3 ml-auto" />}
          </button>
        )}
      </nav>
      ...
      <div className="px-3 py-3 border-t" style={{ borderColor: 'var(--sidebar-border)' }}>
        <div className="flex items-center gap-2 mb-2 px-1">
          <div
            ...
            style={{ background: 'var(--sidebar-accent)', color: 'var(--sidebar-fg)' }}
          >
            {isAdmin ? 'A' : 'U'}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs truncate font-mono" style={{ color: 'var(--sidebar-muted)' }}>
              {principalShort}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <footer className="border-t border-border bg-background py-2 px-4 shrink-0">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-1 text-xs text-muted-foreground">
            <span>© {new Date().getFullYear()} RentIQ Udaipur — Equipment Rental Management</span>
            <span>
              Built with{' '}

...
export default function ChallanRestoreDatesDialog({ open, onClose }: ChallanRestoreDatesDialogProps) {
        ...
        <div className="space-y-4">
          {!canBulkUpload ? (
            ...
            <div className="flex items-center gap-2 text-green-600 p-3 bg-green-50 dark:bg-green-950/20 rounded-lg">
              ...
              <span className="text-sm">Dates restored successfully for {parsedData?.length || 0} challans.</span>
            </div>
          ) : (
            <>
              <p className="text-sm text-muted-foreground">
                Upload a CSV file with Challan ID, Rent Date, and Return Date columns to restore dates.
              </p>
        ...
        <DialogFooter>
          ...
          {!done && canBulkUpload && (
            <Button
              ...
              disabled={!parsedData || parsedData.length === 0 || updateDates.isPending}
            >
              {updateDates.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}

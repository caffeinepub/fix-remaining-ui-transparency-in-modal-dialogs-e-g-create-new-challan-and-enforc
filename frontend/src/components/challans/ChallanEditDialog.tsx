...
import { nanoToDate, dateToNano, toDateInputValue } from '../../utils/dates';
...
export default function ChallanEditDialog({ open, onClose, challan }: ChallanEditDialogProps) {
  ...
  useEffect(() => {
    if (open && challan) {
      ...
      setItems(challan.items.length > 0 ? [...challan.items] : [{ itemName: '', quantity: 1, rate: 0, rentalDays: 1 }]);
      setItemErrors([]);
    }
  }, [open, challan]);
  ...
  const validateItems = () => {
    ...
    items.forEach((item, i) => {
      ...
      const originalQty = challan.items.find(ci => ci.itemName === item.itemName)?.quantity || 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateChallan.mutate({
      ...
      onSuccess: () => { toast.success('Challan updated'); onClose(); },
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={updateChallan.isPending}>
              {updateChallan.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
              Update Challan
            </Button>
          </DialogFooter>
        </form>

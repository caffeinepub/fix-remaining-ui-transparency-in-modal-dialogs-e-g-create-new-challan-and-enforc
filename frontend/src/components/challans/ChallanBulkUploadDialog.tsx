...
import { parseAndValidateChallanCSV } from '../../utils/challanImport';
import { downloadChallanTemplate } from '../../utils/exportToCSV';
import type { Challan } from '../../backend';

interface ChallanBulkUploadDialogProps {
  open: boolean;
  onClose: () => void;
...
export function ChallanBulkUploadDialog({ open, onClose, existingChallans }: ChallanBulkUploadDialogProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const result = parseAndValidateChallanCSV(text, existingChallans);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) handleClose(); }}>
      <DialogContent className="bg-background opacity-100 sm:max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Bulk Upload Challans</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={downloadChallanTemplate}
            >
          ...
          {preview.length > 0 && !uploadDone && (
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">{preview.length} challans ready to upload</p>

...
import { CheckCircle2, Loader2, RefreshCw, Copy, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="border border-border rounded-lg p-4 bg-card text-sm space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-foreground">Backend: {statusLabel}</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      {expanded && (
        <div className="space-y-3 pt-2 border-t border-border">
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="font-mono">{isReady ? 'ready' : isConnecting ? 'initializing' : 'error'}</div>
            <div className="text-muted-foreground">Host</div>
            <div className="font-mono break-all">
              {window.location.hostname.includes('localhost') ? 'http://localhost:4943' : 'https://ic0.app'}
            </div>
          ...
          {testResult && (
            <div className="bg-muted rounded p-2 text-xs font-mono break-all">{testResult}</div>
          )}
        </div>
      )}
    </div>

...
const PAYMENT_MODES = ['CASH', 'ONLINE', 'CHEQUE', 'UPI', 'BANK TRANSFER', 'OTHER'];
...
export default function PaymentFormDialog({ open, onClose }: Props) {
  ...
  useEffect(() => {
    if (open) {
      const now = new Date();
      setId(`PAY${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}${Math.floor(Math.random() * 9000) + 1000}`);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!mode) { toast.error('Please select a payment mode'); return; }
    ...
    addPayment.mutate({
      ...
      onSuccess: () => { toast.success('Payment added'); onClose(); },
      ...
      <DialogContent className="bg-background opacity-100 max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label>Site</Label>
              <Input value={site} onChange={e => setSite(e.target.value)} placeholder="Optional" />
            </div>
          </div>
          <DialogFooter>

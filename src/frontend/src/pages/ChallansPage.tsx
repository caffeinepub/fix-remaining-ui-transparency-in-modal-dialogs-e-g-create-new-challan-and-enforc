...
import { formatDate, nanoToDate } from "../utils/dates";
...
export default function ChallansPage() {
  ...
  const handleMarkReturned = (id: string) => {
    if (!confirm(`Mark challan ${id} as returned? This cannot be undone.`))
      return;
          ...
          <Table>
            ...
            <TableBody>
              {filtered.map((challan) => (
                <TableRow
                  key={challan.id}
                  className={challan.returned ? "opacity-60" : ""}
                >
                  ...
                  <TableCell className="text-sm">
                    {formatDate(nanoToDate(challan.rentDate))}
                  </TableCell>
                  <TableCell>
                    {challan.returned ? (

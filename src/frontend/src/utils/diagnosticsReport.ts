...
export function generateDiagnosticsReport(): DiagnosticsReportData {
  ...
  const blankScreenEvents = log.filter(
    (e) => e.type === "auth-blank-screen",
  ).length;
  const popupBlockedEvents = log.filter(
    (e) => e.type === "auth-popup-blocked",
  ).length;
  const authTimeoutEvents = log.filter((e) => e.type === "auth-timeout").length;

  // Count backend connection events
  const probeAttempts = log.filter(
    (e) => e.type === "actor-probe" || e.type === "connection-probe",
  ).length;
  const successfulProbes = log.filter(
    (e) => e.type === "connection-success",
  ).length;
  const failedProbes = log.filter(
    (e) => e.type === "connection-failure",
  ).length;
  const connectionTimeouts = log.filter(
    (e) => e.type === "actor-timeout",
  ).length;

  // Calculate average response time from successful probes
  const probesWithResponseTime = log.filter(
    (e) =>
      (e.type === "connection-success" || e.type === "connection-failure") &&
      e.context.responseTime !== undefined,
  );
  const averageResponseTime =
    probesWithResponseTime.length > 0
      ? probesWithResponseTime.reduce(
          (sum, e) => sum + (e.context.responseTime || 0),
          0,
        ) / probesWithResponseTime.length
      : null;
  ...
  const lastProbe = [...log]
    .reverse()
    .find(
      (e) => e.type === "connection-success" || e.type === "connection-failure",
    );
  const lastProbeTimestamp = lastProbe
    ? new Date(lastProbe.timestamp).toISOString()
    : null;
...
export function formatDiagnosticsReportAsText(): string {
  ...
  if (
    report.backendConnectionStatus.failedProbes >
    report.backendConnectionStatus.successfulProbes
  ) {

 ...
 */
export function parseAndValidateChallanRestoreCSV(
  csvText: string,
): ParseResult {
  ...
  const hasNewFormat =
    headers.includes("rent date") && headers.includes("return date");
      ...
      errors: [
        {
          row: 1,
          message:
            'Missing required date columns. Expected either "Start Date" and "End Date" OR "Rent Date" and "Return Date"',
        },
      ],
    };
  }
  ...
  const rentDateIndex = hasNewFormat
    ? headers.indexOf("rent date")
    : headers.indexOf("start date");
  const returnDateIndex = hasNewFormat
    ? headers.indexOf("return date")
  ...
  for (let i = 1; i < allRows.length; i++) {
    ...
    try {
      ...
      if (!rentDate) {
        errors.push({
          row: rowNumber,
          message: `Invalid rent/start date format: ${rentDateStr}`,
        });
        continue;
      }
      ...
      if (!returnDate) {
        errors.push({
          row: rowNumber,
          message: `Invalid return/end date format: ${returnDateStr}`,
        });
        continue;
      }

...
export function PettyCashFormDialog({
  ...
  const [transferFromCashEquivalents, setTransferFromCashEquivalents] =
    useState("");
  ...
  const netChange = React.useMemo(() => {
    const ob = Number.parseFloat(openingBalance) || 0;
    const cmd = Number.parseFloat(cashFromMd) || 0;
    const tce = Number.parseFloat(transferFromCashEquivalents) || 0;
    const exp = Number.parseFloat(expenses) || 0;
    const sa = Number.parseFloat(staffAdvance) || 0;
    const hmd = Number.parseFloat(handoverToMd) || 0;
    ...
    if (open) {
      if (editRecord) {
        ...
        setTransferFromCashEquivalents(
          String(editRecord.transferFromCashEquivalents),
        );
  ...
  const handleSubmit = async (print: boolean) => {
    ...
    try {
      ...
      setError(
        err?.message || "Failed to save petty cash record. Please try again.",
      );
    } finally {
      setIsSavingAndPrinting(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-background opacity-100 max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {editRecord ? "Edit Petty Cash Record" : "Add Petty Cash Record"}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="rounded-lg border bg-muted/30 p-4 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span
                className={`font-semibold ${netChange >= 0 ? "text-green-600" : "text-destructive"}`}
              >
            ...
            <div className="flex justify-between text-sm border-t pt-2">
              ...
              <span
                className={`font-bold ${closingBalance >= 0 ? "text-green-600" : "text-destructive"}`}
              >

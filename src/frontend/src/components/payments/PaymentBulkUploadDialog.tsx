...
import { useBulkAddPayments, usePayments } from '../../hooks/useQueries';
...
export default function PaymentBulkUploadDialog({
  open,
  onClose,
}: PaymentBulkUploadDialogProps) {
  const bulkAdd = useBulkAddPayments();
  const { data: existingPayments = [] } = usePayments();
  const { canBulkUpload, disabledReason } = useStaffRestrictions();
  const [file, setFile] = useState<File | null>(null);
  const [parsedData, setParsedData] = useState<Payment[] | null>(null);
                ...
                {parsedData && parsedData.length > 0 && (
                  <Alert className="border-green-200 bg-green-50">
                    ...
                    <AlertTitle className="text-green-800">
                      {parsedData.length} Valid Payment{parsedData.length !== 1 ? 's' : ''}
                    </AlertTitle>
            ...
            <div className="space-y-4">
              {uploadResults.successCount > 0 && (
                <Alert className="border-green-200 bg-green-50">
                  <CheckCircle2 className="h-4 w-4 text-green-600" />
                  <AlertTitle className="text-green-800">
                    Successfully Created {uploadResults.successCount} Payment{uploadResults.successCount !== 1 ? 's' : ''}
                  </AlertTitle>
                </Alert>
              )}
            ...
            <>
              ...
              {parsedData && parsedData.length > 0 && canBulkUpload && (
                <Button onClick={handleImport} disabled={bulkAdd.isPending}>
                  ...
                  {bulkAdd.isPending ? 'Uploading...' : `Upload ${parsedData.length} Payment${parsedData.length !== 1 ? 's' : ''}`}
                </Button>
              )}
            </>
          ) : (

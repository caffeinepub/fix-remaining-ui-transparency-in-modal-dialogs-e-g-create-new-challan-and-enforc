...
export default function InventoryFormDialog({
  ...
  const validate = () => {
    ...
    if (!totalQuantity || Number.parseFloat(totalQuantity) <= 0) {
      toast.error("Total quantity must be > 0");
      return false;
    }
    if (!dailyRate || Number.parseFloat(dailyRate) <= 0) {
      toast.error("Daily rate must be > 0");
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const params = {
      name: name.trim(),
      totalQuantity: Number.parseFloat(totalQuantity),
      dailyRate: Number.parseFloat(dailyRate),
    };

...
export function useAddInventoryItem() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      totalQuantity,
      dailyRate,
    }: { name: string; totalQuantity: number; dailyRate: number }) => {
      if (!actor) throw new Error("Not connected");
...
export function useAddPettyCashAttachment() {
  ...
  return useMutation({
    mutationFn: async ({
      date,
      attachment,
    }: { date: bigint; attachment: PettyCashAttachment }) => {
      if (!actor) throw new Error("Not connected");
      return actor.addAttachmentToPettyCashRecord(date, attachment);
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["pettyCashAttachments", variables.date.toString()],
      });
      queryClient.invalidateQueries({ queryKey: ["pettyCash"] });
    },
  });
...
export function useRemovePettyCashAttachment() {
  ...
  return useMutation({
    mutationFn: async ({
      date,
      attachmentId,
    }: { date: bigint; attachmentId: string }) => {
      if (!actor) throw new Error("Not connected");
...
export function useClientBalances(): ClientBalance[] {
  ...
  return React.useMemo(() => {
    ...
    for (const challan of challans) {
      ...
      const existing = map.get(challan.clientName) || {
        totalRent: 0,
        totalPayments: 0,
      };
      map.set(challan.clientName, {
        ...existing,
    ...
    for (const payment of payments) {
      const existing = map.get(payment.client) || {
        totalRent: 0,
        totalPayments: 0,
      };
      map.set(payment.client, {
        ...existing,
...
export function useDashboardMetrics() {
  ...
  const metrics = React.useMemo(() => {
    ...
    const pettyCash = (
      pettyCashWithAttachments as Array<{ pettyCash: PettyCash }>
    ).map((p) => p.pettyCash);
